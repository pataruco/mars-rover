...
def test_accept_params():
    robot_one = Robot(instructions=robot_one_params["instructions"], position=robot_one_params["position"])
...
def test_move_to_a_new_position():
    ...
    robot_two = Robot(instructions=robot_two_params["instructions"], position=robot_two_params["position"])
...
def test_reports_last_known_coordinates_when_is_lost():
    ...
    assert robot_one.position == {"x": 1, "y": 1, "orientation": "e"}
    assert robot_one.is_lost == False

    assert robot_two.position == {"x": 3, "y": 4, "orientation": "n"}
    assert robot_two.is_lost == True
